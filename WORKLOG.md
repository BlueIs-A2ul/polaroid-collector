...
## 2026-08-30 开发记录（README 重构）

### 改动内容

基于当前项目状态重构 README：

- 修正项目结构树（移除已删除的 `recordService.ts`，补齐 `types/navigation.ts`、`types/report.ts`、暗色模式等）
- 功能特性补充整理中心、暗色模式、EXIF 自动日期、批量合并等缺失项
- 技术栈按实际依赖分组列出（补充 expo-haptics、expo-image-manipulator、react-native-view-shot 等）
- 新增"文档导航"章节，功能细节指向用户手册，避免重复维护

### 文件变更

| 文件 | 变更类型 | 说明 |
|------|----------|------|
| `README.md` | 重构 | 同步当前项目状态 |
| `WORKLOG.md` | 修改 | 新增本条记录 |
